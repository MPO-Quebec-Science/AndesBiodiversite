...
vignette: >
  %\VignetteIndexEntry{Format des données}
...
La fonction `get_biodiv_data()` produit un jeu de données ayant des données quantitative (soit poids et dénombrement).
...
Ces derniers utilisent des lettres majuscules pour les différencier des colonnes DarwinCore.
...
`scientificNameID` [Terme DwC](	http://rs.tdwg.org/dwc/terms/scientificNameID) : Sous la forme `urn.lsid:marinespecies.org:taxname:xxxxxx` où `xxxxxx` est le `AphiaID`. Ce dernier (le `AphiaID`) ayant à une corréspondance directe avec `shared_models_referencecatch.aphia_id` dans ANDES.

`ech_congelo`: `materialEntityType` OU `preparation` on hésite entre ces deux termes darwinCore. A FAIRE!

`occurrenceRemarks` [terme DwC](http://rs.tdwg.org/dwc/terms/occurrenceRemarks) : Commentaires saisies sur le terrain via l'appli Biodiversité. Ce champ à une corréspondance directe avec `shared_models_catch.notes` dans ANDES.
