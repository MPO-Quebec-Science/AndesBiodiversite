...
#' @export
get_biodiv_specimen_data <- function(andes_db_connection) {
  query <- readr::read_file(system.file(
    "sql_queries",
    "biodiv_specimen_data.sql",
  ...
  biodiv_data$scientificNameID <- paste0(
    "urn:lsid:marinespecies.org:taxname:",
    biodiv_data$scientificNameID
  )
  ...
  biodiv_data$eventID <- paste(
    biodiv_data$mission,
    biodiv_data$fieldNumber,
    biodiv_data$sample_number,
    sep = "-"
  )

  biodiv_data$recordNumber <- biodiv_data$catch_id
  ...
  biodiv_data$VALIDATED_SUBSAMPLE_MASS_G <- as.numeric(
    biodiv_data$VALIDATED_SUBSAMPLE_MASS_G
  )
  biodiv_data$VALIDATED_SUBSAMPLE_COUNT <- as.numeric(
    biodiv_data$VALIDATED_SUBSAMPLE_COUNT
  )
  biodiv_data$basket_wt_kg <- as.numeric(biodiv_data$basket_wt_kg)
  biodiv_data$unmeasured_specimen_count <- as.numeric(
    biodiv_data$unmeasured_specimen_count
  )

  has_subsample <- !is.na(biodiv_data$VALIDATED_SUBSAMPLE_COUNT) &
    !is.na(biodiv_data$VALIDATED_SUBSAMPLE_MASS_G)
  has_no_fractional_count <- is.na(biodiv_data$unmeasured_specimen_count) |
    biodiv_data$unmeasured_specimen_count == 0
  ...
  biodiv_data$VALIDATED_FRACTION_MASS_G[
    needs_fraction_extrapolation
  ] <- biodiv_data$basket_wt_kg[needs_fraction_extrapolation]
  biodiv_data$VALIDATED_FRACTION_COUNT[
    needs_fraction_extrapolation
  ] <- biodiv_data$VALIDATED_FRACTION_MASS_G[needs_fraction_extrapolation] *
  ...
  biodiv_data$VALIDATED_FRACTION_COUNT[
    !needs_fraction_extrapolation
  ] <- biodiv_data$unmeasured_specimen_count[!needs_fraction_extrapolation]

  # calculate final values
  biodiv_data$VALIDATED_FINAL_MASS_G <- biodiv_data$VALIDATED_FRACTION_MASS_G *
    biodiv_data$FRACTION_DENOMINATOR
  biodiv_data$VALIDATED_FINAL_COUNT <- biodiv_data$VALIDATED_FRACTION_COUNT *
    biodiv_data$FRACTION_DENOMINATOR

  # convert kg to g
  biodiv_data$VALIDATED_FINAL_MASS_G <- biodiv_data$VALIDATED_FINAL_MASS_G *
    1000
  biodiv_data$VALIDATED_FRACTION_MASS_G <- biodiv_data$VALIDATED_FRACTION_MASS_G *
    1000
  biodiv_data$VALIDATED_SUBSAMPLE_MASS_G <- biodiv_data$VALIDATED_SUBSAMPLE_MASS_G *
    1000
